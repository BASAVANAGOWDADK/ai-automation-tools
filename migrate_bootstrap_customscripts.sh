...
# Bootstrap 4 to Bootstrap 5 Migration Script for Moodle Customscripts
# This script finds and replaces Bootstrap 4 classes with their Bootstrap 5 equivalents
# specifically in the customscripts directory of Moodle.

# Display usage information
echo "========================================================"
echo "Bootstrap 4 to Bootstrap 5 Migration Script for Moodle Customscripts"
echo "========================================================"
echo "This script will update Bootstrap 4 classes to Bootstrap 5 in"
echo "the customscripts directory of your Moodle installation."
echo ""

# Define customscripts directory with Windows path conversion
customscripts_dir="/c/wamp641/www/moodle5/customscripts"

# Verify the directory exists
if [ ! -d "$customscripts_dir" ]; then
    echo "Error: Directory $customscripts_dir does not exist."
    echo "Please update the script with the correct path to your customscripts directory."
    exit 1
fi

# Ask for confirmation
read -p "Are you sure you want to proceed with migrating files in $customscripts_dir? (y/n): " confirm
...
if [[ "$backup" == "y" || "$backup" == "Y" ]]; then
    backup_dir="customscripts_bootstrap_migration_backup_$(date +%Y%m%d_%H%M%S)"
    mkdir -p "$backup_dir"
    echo "Backups will be saved in: $backup_dir"
fi

echo "Starting migration in directory: $customscripts_dir"
...
backup_file() {
    if [[ "$backup" == "y" || "$backup" == "Y" ]]; then
        local file=$1
        local rel_path=$(realpath --relative-to="$customscripts_dir" "$file")
...
# Process files - focus on PHP files first since they're most likely in customscripts
...
done < <(find "$customscripts_dir" -type f -name "*.php" -print0)
...
done < <(find "$customscripts_dir" -type f -name "*.html" -print0)

# Process JavaScript files (if any)
...
done < <(find "$customscripts_dir" -type f -name "*.js" -print0)

# Process template files (if any)
...
done < <(find "$customscripts_dir" -type f -name "*.mustache" -print0)

# Summary
echo ""
echo "========================================================"
echo "Migration Complete for Customscripts directory"
echo "========================================================"
...
if [[ $updated_files -gt 0 ]]; then
    echo ""
    echo "Modified files:"
    find "$customscripts_dir" -type f \( -name "*.php" -o -name "*.mustache" -o -name "*.html" -o -name "*.js" \) -exec grep -l -E "ms-|me-|ps-|pe-|form-select|data-bs-|visually-hidden" {} \; | sort
fi
